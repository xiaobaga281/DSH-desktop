...
安装包是 release 附件，绝不是提交进仓库的文件：git 推送时单个文件超过 100 MiB 就会被 GitHub 拒绝。
...
| 中文产品界面 | 本地化且重排分区的设置壳、可用模型获取、主题与背景、桌宠、MCP 市场页，以及带永久删除语义的归档页 |
...
本页数字来自 2026-09-24 在一台 x64 Windows 机器上的实测：安装包摘要、静默安装与卸载的实际运行、与纯净基座树的文件数比对，以及成对跑的桌面冒烟（安装副本对照源码工作树）。双击向导路径和桌面快捷方式任务没有被实际验证。
