...
基座项目发布的是 CLI 和插件内核，这个构建把它变成桌面产品。2026-09-24 基座发布的是 CLI 与插件内核，这个构建是压在它之上的桌面产品。完整清单见 [IMPROVEMENTS.zh.md](IMPROVEMENTS.zh.md)——它按"能做什么不同的事"分组，并附上从胶囊重算每个数字的命令。
...
| 记忆与决策 | 蒸馏、检索、固化三态，外加记录设计理由的仓库内 Agent 笔记 |
...
上面的冒烟数字是在当天 12:46 构建并安装到 D 盘的副本上量到的。现在挂着的这份（14:38）在它之上加了市场相关的修复，而这一步是核对包内文件确认的，没有再装第二遍。
